...
export function getSpecialistById(id: number) {
  return INITIAL_SPECIALISTS.find(master => master.id === id) || INITIAL_SPECIALISTS[0];
}

export function getOrderById(id: number) {
  return INITIAL_ORDERS.find(order => order.id === id) || INITIAL_ORDERS[0];
}
